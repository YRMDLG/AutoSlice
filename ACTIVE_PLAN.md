...
plan_version: 1
plan_status: active
current_action: UI-U1
current_action_status: pending
execution_mode: single-action-stop
last_completed_action: TASK-T3

## 执行规则

- 当前处于 active，`TASK-T3` 已完成实现、验证、提交、Windows full hermetic 与 Linux logic-only CI 双绿并登记；当前 Action 为 `UI-U1`，状态为 pending。
...
- 一次只执行一个 Action；本轮仅登记 `TASK-T3`，登记后立即停止，不得开始 `UI-U1`。
- `FE-PREP-1`、`FE-PREP-2`、`FE-S0`、`FE-S1`、`FE-S2`、`FE-S3`、`FE-S4`、`FE-S5`、`ASR-A1`、`ASR-A2`、`COVER-C0`、`COVER-C1`、`COVER-C2`、`COVER-C3`、`TASK-T1`、`TASK-T2`、`TASK-T3` 均已登记完成，不得重复运行；`UI-U1` 当前为 pending，须在后续单独授权后执行。
