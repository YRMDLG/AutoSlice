...
plan_version: 1
plan_status: active
current_action: TASK-T1
current_action_status: pending
execution_mode: single-action-stop
last_completed_action: COVER-C3

## 执行规则

- 当前处于 active，`COVER-C3` 已完成实现、验证、提交、Windows full hermetic 与 Linux logic-only CI 双绿并登记；当前 Action 为 `TASK-T1`，状态为 pending。
...
- 一次只执行一个 Action；本轮仅登记 `COVER-C3`，登记后立即停止，不得开始 `TASK-T1`。
- `FE-PREP-1`、`FE-PREP-2`、`FE-S0`、`FE-S1`、`FE-S2`、`FE-S3`、`FE-S4`、`FE-S5`、`ASR-A1`、`ASR-A2`、`COVER-C0`、`COVER-C1`、`COVER-C2`、`COVER-C3` 均已登记完成，不得重复运行；下一轮从 `TASK-T1` 开始。
