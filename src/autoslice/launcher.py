...
def _run_gpu_child(
    ...
    source_entry = (
        SOURCE_WORKSPACE_ROOT / "启动.py"
        if SOURCE_WORKSPACE_ROOT is not None
        else None
    )
    command = [str(runtime_python)]
    if source_entry is not None and source_entry.is_file():
        command.append(str(source_entry))
    else:
        command.extend(("-m", "autoslice.launcher"))
    command.extend(list(argv) if argv is not None else sys.argv[1:])
    try:
