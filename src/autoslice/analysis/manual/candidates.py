...
def is_manual_merge_target(topic):
    ...
    return not any(keyword in compact for keyword in clip_policy.UNCUTTABLE_CONTENT_KEYWORDS)
...
def merge_manual_timeline_topics(topics, entries):
    ...
    for topic in topics:
        if not is_manual_merge_target(topic):
            continue
        matched = [entry for entry in entries if manual_entry_matches_topic(entry, topic)]
        if not matched:
            continue
    ...
    for entry in entries:
        ...
        if any(entry in (topic.get("manual_timeline") or []) for topic in topics):
            continue
        if any(
            is_manual_merge_target(topic) and manual_entry_matches_topic(entry, topic)
            for topic in topics
        ):
            continue
        topic_start = (
            ...
            if optimized
                ...
                0,
                int(entry["start"]) - timeline_analysis.MANUAL_TIMELINE_TOPIC_PRE_SEC,
            )
        )
        topic_end = (
            max(topic_start + 1, int(entry.get("end", topic_start + 1)))
            if optimized
            else int(entry["start"]) + timeline_analysis.MANUAL_TIMELINE_TOPIC_POST_SEC
        )
        topic = {
            "start": topic_start,
            "end": topic_end,
...
def topics_from_manual_timeline(
    ...
    for group in groups:
        ...
        explicit_end = (
            group[0].get("end") if len(group) == 1 and group[0].get("explicit_range") else None
        )
...
def manual_evidence_line(entry):
    ...
    return (
        f"{prefix}：{timecode.format_elapsed(int(entry.get('start', 0)))} {entry.get('text', '')}"
    )
...
def sanitize_optimized_manual_entry(entry):
    ...
    fixed = dict(entry or {})
    original_entries = [
        dict(item) for item in fixed.get("original_entries") or [] if isinstance(item, dict)
    ]
