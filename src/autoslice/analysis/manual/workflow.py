...
def format_manual_entry_for_prompt(entry):
    ...
    if entry.get("end") is not None and int(entry["end"]) > int(entry["start"]):
        elapsed_label = f"{elapsed_label}-{timecode.format_elapsed(entry['end'])}"
...
def manual_timeline_info_for_chunk(entries, chunk_start, chunk_end, limit=12):
    ...
    if len(selected) < limit:
        selected.extend(item for item in nearby if item not in selected)
...
def optimized_manual_entries_from_topics(topics):
    ...
    for topic in topics or []:
        ...
        stars = max(
            [int(topic.get("manual_stars", 0))] + [item["stars"] for item in original_entries]
        )
        summary = []
...
def optimized_entry_needs_retry(entry):
    ...
    if manual_enrichment.is_manual_ai_placeholder(entry.get("text")):
        return True
    return any(
        manual_enrichment.is_manual_ai_placeholder(title_analysis._strip_body_prefix(point))
...
def topic_from_optimized_entry(entry, srt_segments, peaks):
    ...
    if not any(str(line).startswith("·弹幕依据：") for line in body):
        body[:0] = candidate_evidence.topic_danmaku_reference_lines(start, end, peaks or [])
    if not any(str(line).startswith("·字幕核查：") for line in body):
        body.extend(candidate_evidence.topic_srt_summary_lines(start, end, srt_segments or []))
...
def retry_optimized_timeline_entries(
    ...
    accepted_entries = [
        dict(entry) for entry in entries or [] if not optimized_entry_needs_retry(entry)
    ]
    retry_topics = [
    ...
    optimized_entries = accepted_entries + optimized_manual_entries_from_topics(retry_topics)
...
def optimize_manual_timeline(
    ...
    if not entries:
        return [], None
    aligned_entries = timeline_analysis.align_manual_timeline_entries_to_srt(entries, srt_segments)
    topics = manual_candidates.topics_from_manual_timeline(
        aligned_entries,
        srt_segments=srt_segments,
        peaks=peaks,
        max_gap_sec=timeline_analysis.MANUAL_TIMELINE_OPTIMIZE_GAP_SEC,
        max_group_duration_sec=(timeline_analysis.MANUAL_TIMELINE_OPTIMIZE_MAX_GROUP_SEC),
    )
    warning = manual_review.enrich_manual_topics_in_batches(
        topics,
        streamer_name=streamer_name,
