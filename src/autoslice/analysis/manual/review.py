...
FACADE_EXPORTS = {
    "_build_manual_topic_enrichment_prompt": ("build_manual_topic_enrichment_prompt"),
...
def build_manual_topic_enrichment_prompt(
    ...
    for index, topic in enumerate(topics or [], 1):
        ...
        subtitle_evidence = [line for line in evidence if line.startswith("字幕核查：")]
        manual_evidence = [line for line in evidence if line.startswith(("人工时间轴", "时间轴："))]
        density_evidence = [line for line in evidence if line.startswith("弹幕依据：")]
        candidates.append(
            {
                "id": index,
...
def enrich_manual_topics_with_llm(
    ...
    if not isinstance(raw_topics, list):
        raise llm_gateway.LLMStructuredOutputError("人工时间轴 AI 复核未返回 topics 数组")
    ...
    if not updated:
        raise llm_gateway.LLMStructuredOutputError("人工时间轴 AI 复核没有返回可用话题")
    topics[:] = sorted(
        enriched_topics,
    ...
    with ThreadPoolExecutor(
        ...
        for job, future in zip(jobs, futures):
            ...
            if report_progress:
                ...
                report_progress(
                    f"{progress_label}完成 ({completed_batches}/{total_batches})",
                    current_step,
                    100,
                )
...
def validate_unmatched_manual_topics(
    ...
    if warning:
        return f"人工时间轴补充项部分复核失败；未核验条目仅写入报告且不会自动切片：{warning}"
    return None
