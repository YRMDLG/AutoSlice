...
def extract_video_start_datetime(video_path):
    ...
    for text in candidates:
        for pattern in patterns:
            ...
            try:
                return datetime(
                    parts["y"], parts["m"], parts["d"], parts["h"], parts["mi"], parts["s"]
                )
            except ValueError:
                continue
    return None
...
def manual_timeline_doc_candidates(video_start, timeline_dir=MANUAL_TIMELINE_DIR):
...
def find_manual_timeline_doc(video_path, timeline_dir=MANUAL_TIMELINE_DIR):
    """自动查找时间轴目录中和录播日期匹配的 docx。"""
    video_start = extract_video_start_datetime(video_path)
    for path in manual_timeline_doc_candidates(video_start, timeline_dir):
...
def parse_manual_timeline_lines(lines, video_start):
    ...
    for line, line_period_start, line_period_end in expanded_lines:
        ...
        try:
            if line_period_start and line_period_end:
                event_dt = datetime(
                    line_period_start.year,
                    line_period_start.month,
                    line_period_start.day,
                    *event_time,
                )
                ...
                candidates = [
                    datetime.combine(
                        video_start.date() + timedelta(days=offset), datetime.min.time()
                    ).replace(hour=event_time[0], minute=event_time[1], second=event_time[2])
                    for offset in (-1, 0, 1)
                ]
                event_dt = min(
                    candidates, key=lambda item: abs((item - video_start).total_seconds())
                )
        except ValueError:
            continue
...
def parse_elapsed_timeline_report_lines(lines):
...
def filter_manual_timeline_entries(
    entries, video_duration, end_margin_sec=MANUAL_TIMELINE_END_MARGIN_SEC
):
...
def load_manual_timeline(video_path, timeline_dir=MANUAL_TIMELINE_DIR, manual_timeline_path=None):
    ...
    if manual_timeline_path:
        ...
        doc_path = find_manual_timeline_doc(video_path, timeline_dir)
    if not doc_path:
        return {
            "path": None,
            "entries": [],
    ...
    entries = parse_manual_timeline_lines(lines, video_start)
    time_basis = "wall_clock_converted_to_video_elapsed_seconds" if entries else None
    if not entries:
        entries = parse_elapsed_timeline_report_lines(lines)
        if entries:
            time_basis = "video_elapsed_seconds"
    return {
        "path": doc_path,
...
def manual_timeline_summary(manual_timeline):
    ...
    return {
        ...
        "video_start": video_start,
        "time_basis": manual_timeline.get("time_basis")
        or (
...
def manual_alignment_text(text):
    return "".join(re.findall(r"[\u4e00-\u9fffA-Za-z0-9]+", str(text or ""))).lower()


def manual_alignment_score(reference, candidate):
    """用二元字组和最长公共片段匹配人工概述与噪声 ASR。"""
    reference = manual_alignment_text(reference)
    candidate = manual_alignment_text(candidate)
...
def manual_semantic_core(text):
    """移除称呼和叙述套话，只保留可用于核对事件的词面锚点。"""
    value = manual_alignment_text(text)
    ...
    for phrase in generic_terms:
        value = value.replace(manual_alignment_text(phrase), "")
    return value


def manual_text_supports_candidate(reference, candidate):
    """保守判断人工原句是否真的支持 AI 改写，而非只处于附近时间。"""
    if manual_alignment_score(reference, candidate) >= MANUAL_TIMELINE_GROUNDING_MIN_SCORE:
        return True
    reference_core = manual_semantic_core(reference)
    candidate_core = manual_semantic_core(candidate)
    ...
    if match.size >= 3:
        return True
    reference_grams = {
        reference_core[index : index + 2] for index in range(len(reference_core) - 1)
    }
    candidate_grams = {
        candidate_core[index : index + 2] for index in range(len(candidate_core) - 1)
    }
    shared = (reference_grams & candidate_grams) - _MANUAL_SEMANTIC_BIGRAM_STOPWORDS
    return len(shared) >= 2


def srt_alignment_windows(srt_segments):
...
def align_manual_timeline_entries_to_srt(entries, srt_segments):
    """在原墙钟点前后十分钟搜索字幕证据，修正人工记录的粗略锚点。"""
    windows = srt_alignment_windows(srt_segments)
    if not windows:
    ...
    for entry in entries or []:
        ...
        for window in nearby:
            content_score = manual_alignment_score(entry.get("text", ""), window[2])
