...
def enrich_manual_topic_from_item(topic, item):
    ...
    enriched = dict(topic)
    raw_title = title_analysis._clean_topic_title(
        str(item.get("title", topic.get("title", "")))
    )
    if is_manual_ai_placeholder(raw_title) or title_analysis._is_incomplete_ai_title(
        raw_title
    ):
        raw_title = ""
