...
def _build_review_jobs(
    ...
    return retry_jobs, total_batches


def _apply_review_response(job, response, last_errors):
    """应用一批复核结果，返回需要缩小批次重试的原始候选。"""
    ...
    for item_id, (original, candidate) in enumerate(
        ...
        if not topic_response.json_can_slice(item.get("valid"), ""):
        ...
        interest_score = round(
            min(100.0, base_interest_score + timeline_star_bonus),
            1,
        )
...
def review_peak_selected_topics(
    ...
    for batch_size, round_label in review_rounds:
        if not unresolved:
            break
        retry_items = []
        jobs, total_batches = _build_review_jobs(
            unresolved,
            batch_size,
            srt_segments,
            high_energy_peaks,
        ...
        with ThreadPoolExecutor(
            ...
            for job, future in zip(jobs, futures):
                ...
                try:
                    ...
                    retry_items.extend(
                        _apply_review_response(job, response, last_errors)
                    )
