...
FACADE_EXPORTS = {
    '_format_manual_entry_for_prompt': '_format_manual_entry_for_prompt',
    '_manual_timeline_info_for_chunk': '_manual_timeline_info_for_chunk',
    ...
    '_batch_warning_text': '_batch_warning_text',
...
def retry_optimized_timeline_entries(
    ...
    accepted_entries = [dict(entry) for entry in entries or [] if not optimized_entry_needs_retry(entry)]
    retry_topics = [
...
def prepare_optimized_manual_timeline(
    ...
    if reusable_artifact:
        retry_count = sum(
            optimized_entry_needs_retry(entry)
        ...
        if retry_incomplete_artifact:
            ...
            optimized_entries, warning = retry_optimized_timeline_entries(
        ...
        def save_fresh_checkpoint(processed_topics, remaining_topics, warnings):
            ...
            for topic in remaining_topics:
                ...
                pending["reference_only"] = True
                pending_topics.append(pending)
            checkpoint_entries = optimized_manual_entries_from_topics(
                list(processed_topics) + pending_topics
            )
            write_checkpoint(
                checkpoint_entries,
        ...
        optimized_entries, warning = optimize_manual_timeline(
            raw_entries,
            srt_segments=srt_segments,
            peaks=peaks,
            streamer_name=streamer_name,
