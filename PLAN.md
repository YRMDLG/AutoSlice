...
### Action 19.4: 全量回归与实际检查点恢复 [待办]

| 字段 | 值 |
|------|-----|
| 状态 | 待办 |
| 依赖 | Action 19.3 |
| 描述 | 运行完整测试；使用 20260714 已生成 SRT、FunASR 检查点和优化时间轴检查点恢复混合流水线，确认不重新转录、503 最长共享等待 11 秒、服务恢复时可继续分析，服务持续不可用时明确失败而不产出伪完成报告。 |
| 涉及文件 | `PLAN.md` |
| 验证命令 | `python -B -m unittest test_subtitle_workflow.py test_app.py test_topic_engine.py test_launcher.py -v` |
| 验证预期 | 全量测试通过；真实恢复日志与产物符合描述，源视频、SRT 和人工 DOCX 哈希不变。 |
| 重试次数 | 0/10 |
| 完成时间 |  |
| 备注 |  |
