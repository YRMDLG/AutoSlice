...
命令，并保留 `python 启动.py`。AutoSlice 的 owner、Flask 模板和静态资源已经迁入
`src/autoslice`；完整 wheel 脱离仓库运行仍需等待 AutoCover 独立包迁移和安装态门禁。
...
| 指标 | 重构起点 | 当前快照 | 解读 |
|---|---:|---:|---|
| 产品 Python 模块 | 31 | 75 | 能力拆成显式模块，同时新增 `src` owner、安全、任务、路径、打包入口和兼容 alias |
| 产品源码行数 | 25,905 | 33,200 | 增长来自新增契约、安全、功能和安装验证；不能把总行数当作单一质量指标 |
| 本地 import 边 | 41 | 135 | 分层与兼容边界更显式，架构测试保证方向和无环 |
| 依赖环 | 1 | 0 | `topic_engine` 与字幕工作流的循环已消除 |
| 重复顶层定义 | 0 | 0 | 防止“新模块已导入、旧实现又在后面覆盖” |
| 测试对私有符号的 patch | 138 | 17 | 降低约 88%，测试优先走稳定 seam |
| `topic_engine.py` | 12,337 行 | 695 行 | 缩减约 94%，现为兼容 façade 与薄 CLI |
...
| 模块 | 行数 | 最长函数 |
|---|---:|---:|
| `src/autoslice/analysis/candidates.py` | 5,363 | 264 |
...
| `src/autoslice/pipeline.py` | 1,698 | 415 |
| `src/autoslice/reporting.py` | 1,090 | 183 |
| `src/autoslice/analysis/titles.py` | 1,086 | 117 |
| `src/autoslice/llm/transport.py` | 915 | 124 |
| `src/autoslice/topic_engine.py` | 695 | 22 |
| `src/autoslice/slicing.py` | 636 | 292 |
