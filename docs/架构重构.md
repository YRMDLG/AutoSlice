...
这不等于“长期重构和最终验收已经全部完成”。当前仍需完成公开文档、兼容表面、Web/字幕工作流、AutoCover 状态、配置依赖、安全清理和最终真实录播的后期收口。不能因为历史真实验收曾通过，就宣称后续架构与任务状态修改已经完成最终真实验收。
...
| 指标 | 重构起点 | 当前快照 | 说明 |
|---|---:|---:|---|
| 生产 Python 模块 | 31 | 170 | 真实 owner、兼容层、AutoCover 和发布脚本均显式计入 |
| 生产源码行数 | 25,905 | 37,464 | 新增功能、契约、安全和测试入口使总量增加，不能单独代表债务 |
| 顶层函数 | 未建立统一基线 | 910 | 包含公开入口、内部 owner 和发布脚本 |
| 顶层类 | 未建立统一基线 | 87 | 主要是配置、任务、结果和工作区模型 |
| 本地 import 边 | 41 | 441 | 分层更显式，由无环护栏约束 |
...
| `src/autoslice/analysis/topic/analysis.py` | 683 | `analyze_topic_chunks`，288 行 |
| `src/autoslice/analysis/review/workflow.py` | 307 | `review_peak_selected_topics`，287 行 |
| `src/autoslice/analysis/review/decisions.py` | 545 | `apply_danmaku_slice_decisions`，142 行 |
| `src/autoslice/subtitle_workflow.py` | 2,366 | `suggest_subtitle_corrections`，151 行 |
| `src/autoslice/web/app.py` | 2,388 | `update_task`，133 行 |
| `src/autoslice/transcription/service.py` | 105 | 无顶层真实函数；兼容 façade |
| `src/autoslice/transcription/workflow.py` | 374 | `ensure_srt`，133 行 |
| `src/autoslice/pipeline.py` | 1,092 | `run_pipeline_impl`，297 行 |
| `src/autoslice/pipeline.py` 重试入口 | 同上 | `retry_clip_review_from_artifacts_impl`，234 行 |
| `src/autoslice/reporting.py` | 1,095 | `organize_existing_artifacts`，183 行 |
...
| `src/autoslice_cover/app.py` | 984 | `create_app`，408 行 |
| `src/autoslice_cover/renderer.py` | 1,245 | `render_cover`，146 行 |
...
截至当前文档同步 Action 的已知本地基线：

```text
Hermetic：1230 项通过，跳过 2 项
Linux logic-only：194 项通过
架构测试：81 项通过
公开编译：267 个文件
依赖环：0
重复顶层定义：0
...
Windows CI 运行完整 hermetic 套件；Ubuntu CI 只运行不依赖真实模型、FFmpeg、字体、浏览器和用户媒体的逻辑白名单。Linux 逻辑测试通过不代表 Linux 已支持完整 GPU、字幕压制和媒体工作流；当前完整主平台仍是 Windows 10/11，macOS 未验证。

两项跳过必须是明确依赖可选环境的测试，不能静默跳过核心业务逻辑。测试数量会随着后续 Action 增加，本文记录的是当前检查点，不应手工固定为永久常量。
...
完成后期架构、兼容、安全、清理和文档 Action 后，仍必须重新执行用户指定的有限真实录播验收：长录播只验证字幕读取或必要转录、弹幕、LLM、人工时间轴、候选、边界、报告、检查点、切片和续跑；字幕识别、校对和压制只对最终短片或代表性短样本执行，严禁对数小时录播做整场字幕压制。

## 11. 剩余技术债务和后续顺序

当前剩余工作按风险和依赖顺序处理：

1. **公开文档同步。** README、架构、配置、工作流和故障排查必须与当前 owner、任务结果和真实支持范围一致。
2. **主流水线与 retry 最终证据审计。** 确认剩余代码是稳定编排，不存在可以证明的重复实现；不为了压行数新增空阶段。
3. **兼容 façade 最终统计。** 对每个兼容符号统计生产、测试、文档和旧脚本引用，只删除确认没有兼容价值的导出。
4. **字幕工作流和 Web 审计。** 只处理有复现、测试或代码证据的双实现、状态错误和维护风险，不因文件较长进行大重写。
5. **AutoCover 状态复核。** 保持磁盘草稿、浏览器崩溃恢复和标签页恢复的分工，验证旧预览不会覆盖当前编辑状态。
6. **配置、依赖、媒体格式和错误提示。** 显式声明直接依赖，使扫描、输入、输出和文档支持范围一致，并提供可执行中文修复提示。
7. **安全清理。** 只删除可再生成缓存和已确认无未合并工作的废弃 worktree；不删除用户配置、草稿、媒体、字幕、时间轴、模型、字体、贴图、报告或检查点。
8. **最终真实验收。** 重新验证指定长录播、最终短片字幕和重复续跑幂等性。
9. **最终发布复核。** 从干净 clone 验证文档、配置示例、启动入口、wheel、敏感信息和 GitHub 文件树。
