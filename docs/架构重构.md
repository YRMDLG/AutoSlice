...
| 生产源码行数 | 25,905 | 37,876 | 新增功能、契约、安全和测试入口使总量增加，不能单独代表债务 |
| 顶层函数 | 未建立统一基线 | 914 | 包含公开入口、内部 owner 和发布脚本 |
| 顶层类 | 未建立统一基线 | 87 | 主要是配置、任务、结果和工作区模型 |
| 本地 import 边 | 41 | 443 | 分层更显式，由无环护栏约束 |
...
| `src/autoslice/analysis/review/workflow.py` | 339 | `review_peak_selected_topics`，136 行；批次构建 57 行；结果应用 122 行 |
