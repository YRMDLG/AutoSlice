...
静态资源已经分别迁入 `src/autoslice`、`src/autoslice_cover`；完整 wheel 脱离
仓库运行仍需通过下一阶段的安装态门禁。
...
| 指标 | 重构起点 | 当前快照 | 解读 |
|---|---:|---:|---|
| 产品 Python 模块 | 31 | 88 | AutoSlice/AutoCover 均有显式 `src` owner，旧入口拆成无业务实现的兼容 alias |
| 产品源码行数 | 25,905 | 33,183 | 增长来自新增契约、安全、功能和安装验证；不能把总行数当作单一质量指标 |
| 本地 import 边 | 41 | 149 | 两个生产包的分层与兼容边界更显式，架构测试保证方向和无环 |
