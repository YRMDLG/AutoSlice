...
- `topic_engine.py` 从业务单体收敛为 688 行兼容 façade，文件内没有函数或类实现；
...
这不等于“技术债务清零”。`autoslice/analysis/candidates.py`、`autoslice/pipeline.py` 和 `autoslice/transcription/service.py` 仍偏大，最长函数仍有 415 行；这些是后续小步拆分对象，不应再次采用整文件重写。

仓库现在通过 `pyproject.toml` 声明 editable 安装、统一依赖和 `autoslice`
命令，并保留 `python 启动.py`。在完成 `src/` 与 Flask 资源迁移前，安装契约
只覆盖克隆工作区中的 editable 模式，不宣称 wheel 可以脱离仓库运行。

## 2. 当前模块关系

```mermaid
flowchart TD
    UI["Flask Web / app.py"] --> Facade["topic_engine.py<br/>兼容 façade"]
    Facade --> Pipeline["autoslice/pipeline.py<br/>编排与续跑"]
    ...
    Pipeline --> Reporting["autoslice/reporting.py<br/>报告与整理包"]
    Pipeline --> Slicing["autoslice/slicing.py<br/>FFmpeg 与切片复用"]
    ...
    Subtitle --> LLM
    Cover["autocover_tool/"] --> Security
```

依赖方向的核心规则是：`autoslice.*` 底层模块不得反向导入 `app.py`、`subtitle_workflow.py` 或 `topic_engine.py`。`topic_engine.py` 只向旧调用方重导出对象；新脚本应直接依赖 `autoslice.pipeline`、`autoslice.slicing` 等 owner。
...
| 候选、边界和复核 | `autoslice/analysis/candidates.py` | 负责投稿价值、上下文、去重、非重叠和语义收尾 |
...
| 工作区、包资源与可写状态路径 | `autoslice/paths.py`、`autocover_tool/autocover/paths.py` | 源码态兼容仓库目录，安装态写入用户数据目录，不依赖当前工作目录 |

## 4. 兼容层的边界

### `topic_engine.py`

旧代码仍可从 `topic_engine` 导入公开符号。文件中的符号直接绑定到 owner 对象，并由身份测试保证不会在文件后部出现第二套同名实现。它目前仍有较多兼容重导出，所以 688 行不能理解为理想的最终公开 API；新增代码不应继续扩大该表面。
...
| 指标 | 重构起点 | 当前快照 | 解读 |
|---|---:|---:|---|
| 产品 Python 模块 | 31 | 59 | 能力拆成显式模块，同时新增安全、任务、路径、打包入口和验证工具 |
| 产品源码行数 | 25,905 | 33,024 | 增长来自新增契约、安全、功能和安装验证；不能把总行数当作单一质量指标 |
| 本地 import 边 | 41 | 116 | 分层更显式，架构测试保证方向和无环 |
| 依赖环 | 1 | 0 | `topic_engine` 与字幕工作流的循环已消除 |
| 重复顶层定义 | 0 | 0 | 防止“新模块已导入、旧实现又在后面覆盖” |
| 测试对私有符号的 patch | 138 | 17 | 降低约 88%，测试优先走稳定 seam |
| `topic_engine.py` | 12,337 行 | 688 行 | 缩减约 94%，现为纯兼容 façade |
...
| 模块 | 行数 | 最长函数 |
|---|---:|---:|
| `autoslice/analysis/candidates.py` | 5,363 | 264 |
| `subtitle_workflow.py` | 2,366 | 151 |
| `app.py` | 2,204 | 137 |
| `autoslice/transcription/service.py` | 1,937 | 364 |
| `autoslice/pipeline.py` | 1,704 | 415 |
| `autoslice/reporting.py` | 1,090 | 183 |
| `autoslice/analysis/titles.py` | 1,086 | 117 |
| `autoslice/llm/transport.py` | 915 | 124 |
| `topic_engine.py` | 688 | 无本地函数 |
| `autoslice/slicing.py` | 636 | 292 |
| `core.py` | 81 | 35 |
...
AutoSlice 测试按职责存放：`tests/unit/` 是可隔离服务与纯逻辑测试，
`tests/integration/` 覆盖 Flask、启动器、字幕和完整话题流水线，
`tests/architecture/` 固定依赖方向与外部边界，`tests/support/` 只保存
测试运行器和用例共用的护栏。AutoCover 保持自己的
`autocover_tool/tests/`，统一 hermetic runner 会同时发现两套测试。
...
1. **候选模块仍偏大。** `candidates.py` 有 5,363 行，候选清洗、价值复核、边界、去重和部分检查点逻辑仍可按稳定数据契约继续拆分。
