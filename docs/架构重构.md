...
| FunASR 模型、SRT 和转录编排 | `autoslice/transcription/service.py` | 分块失败不会留下可复用的残缺正式 SRT |
| FunASR 结果契约 | `autoslice/transcription/results.py` | 统一 Nano/Paraformer 时间戳、检查点结构与 CAM++ 主要说话人选择 |
...
| 指标 | 重构起点 | 当前快照 | 解读 |
|---|---:|---:|---|
| 产品 Python 模块 | 31 | 94 | AutoSlice/AutoCover 均有显式 `src` owner，旧入口拆成无业务实现的兼容 alias |
| 产品源码行数 | 25,905 | 33,854 | 增长来自新增契约、安全、功能和安装验证；不能把总行数当作单一质量指标 |
| 本地 import 边 | 41 | 171 | 两个生产包的分层与兼容边界更显式，架构测试保证方向和无环 |
| 依赖环 | 1 | 0 | `topic_engine` 与字幕工作流的循环已消除 |
| 重复顶层定义 | 0 | 0 | 防止“新模块已导入、旧实现又在后面覆盖” |
| 测试对私有符号的 patch | 138 | 17 | 降低约 88%，测试优先走稳定 seam |
| `topic_engine.py` | 12,337 行 | 704 行 | 缩减约 94%，现为兼容 façade 与薄 CLI |
...
| `src/autoslice/transcription/service.py` | 1,740 | 364 |
| `src/autoslice/transcription/results.py` | 232 | 129 |
...
| `src/autoslice/topic_engine.py` | 704 | 22 |
...
3. **ASR 编排仍承担较多职责。** `ensure_srt()` 为 364 行；原始结果契约已经独立，但模型装载、分块执行和结果提交仍需继续小步拆分，并保持原子 SRT 契约。
